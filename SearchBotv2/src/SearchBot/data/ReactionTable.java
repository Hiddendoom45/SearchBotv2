...
public class ReactionTable {
	...
	private void createTable() throws SQLException {
		try (Connection conn = h2Connector.getConnectionPool().getConnection()) {
			String s = "";
			// generate the columns for the emotes based on the EmoteValues
			// enum, textName is the column friendly string
			for (EmoteValues e : EmoteValues.values()) {
				s += ", " + e.textName + " INT";
			}
			conn.createStatement().execute("CREATE TABLE MJREACTION IF NOT EXISTS (ID BIGINT PRIMARY KEY,USER_ID BIGINT" + s + " )");
		}
	}
	...
	public void saveMJFlag(MJFlag f) throws SQLException {
		try (Connection conn = h2Connector.getConnectionPool().getConnection()) {
			...
			PreparedStatement ps = conn
					.prepareStatement(
							"MERGE INTO MJREACTION (ID, USER_ID"
									+ Arrays.stream(EmoteValues.values()).map(e -> e.emoteText).collect(
											Collectors.joining(","))
									+ ")  KEY(ID) VALUES(?,"
